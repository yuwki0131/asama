"""Headless Blender entry point for the WALLROAD family (V-05 phases).

Twin of render_asset.py that resolves models from the isolated
render_asset_lib.wallroad registry instead of the static one, so the
static pipeline (render_asset.py / registry.py and its cached assets) is
untouched. Same camera rig, same CLI contract:

    blender --background --factory-startup --python render_wallroad_asset.py -- \
        --model wall-plaster-connected-0101-p1 \
        --canvas 64x96 \
        --anchor 32,80 \
...
from render_asset_lib.wallroad.registry import resolve_model, WALLROAD_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown wallroad model: {args.model}; patterns: {WALLROAD_MODEL_PATTERNS}"
        )
