"""Headless Blender entry point for the FENCE family.

Twin of render_asset.py that resolves models from the isolated
render_asset_lib.fence registry instead of the static one, so the static
pipeline (render_asset.py / registry.py and its cached assets) is untouched.
Same camera rig, same CLI contract:

    blender --background --factory-startup --python render_fence_asset.py -- \
        --model fence-wood-connected-1010 \
        --canvas 64x96 \
        --anchor 32,80 \
...
from render_asset_lib.fence.registry import resolve_model, FENCE_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown fence model: {args.model}; patterns: {FENCE_MODEL_PATTERNS}"
        )
