"""Headless Blender entry point for the SHORE (terrain water) family.
...
Same camera rig, same CLI contract:
    ...
    blender --background --factory-startup --python render_shore_asset.py -- \
        --model terrain-water-connected-0101 \
        --canvas 64x40 \
        --anchor 32,16 \
...
from render_asset_lib.shore.registry import resolve_model, SHORE_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown shore model: {args.model}; patterns: {SHORE_MODEL_PATTERNS}"
        )
