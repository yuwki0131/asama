"""Headless Blender entry point for the MACHIYA family.

Twin of render_asset.py that resolves models from the isolated
render_asset_lib.machiya registry instead of the static one, so the static
pipeline (render_asset.py / registry.py and its cached assets) is untouched.
Same camera rig, same CLI contract:

    blender --background --factory-startup --python render_machiya_asset.py -- \
        --model building-machiya-v2-nw_se \
        --canvas 192x160 \
        --anchor 112,128 \
...
from render_asset_lib.machiya.registry import resolve_model, MACHIYA_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown machiya model: {args.model}; patterns: {MACHIYA_MODEL_PATTERNS}"
        )
