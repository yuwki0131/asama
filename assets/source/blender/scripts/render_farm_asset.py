"""Headless Blender entry point for the FARM (terrain wetland) family.
...
Same camera rig, same CLI contract:
    ...
    blender --background --factory-startup --python render_farm_asset.py -- \
        --model terrain-farm-connected-0101 \
        --canvas 64x32 \
        --anchor 32,16 \
...
from render_asset_lib.farm.registry import resolve_model, FARM_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown farm model: {args.model}; patterns: {FARM_MODEL_PATTERNS}"
        )
