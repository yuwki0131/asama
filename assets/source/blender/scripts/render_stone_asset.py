"""Headless Blender entry point for the STONE (terrain rocky ridge) family.

Twin of render_asset.py that resolves models from the isolated
render_asset_lib.shore registry instead of the static one, so the static
pipeline (render_asset.py / registry.py and its cached assets) is untouched.
Same camera rig, same CLI contract:

    blender --background --factory-startup --python render_stone_asset.py -- \
        --model terrain-stone-connected-0101 \
        --canvas 64x32 \
        --anchor 32,16 \
...
from render_asset_lib.stone.registry import resolve_model, STONE_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown stone model: {args.model}; patterns: {STONE_MODEL_PATTERNS}"
        )
