"""Headless Blender entry point for the TRENCH (moat/river) family.

Twin of render_asset.py that resolves models from the isolated
render_asset_lib.trench registry instead of the static one, so the static
pipeline (render_asset.py / registry.py and its cached assets) is untouched.
Same camera rig, same CLI contract:

    blender --background --factory-startup --python render_trench_asset.py -- \
        --model water-moat-connected-0101 \
        --canvas 64x48 \
        --anchor 32,16 \
...
from render_asset_lib.trench.builders import disable_holdout_shadows
from render_asset_lib.trench.registry import resolve_model, TRENCH_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown trench model: {args.model}; patterns: {TRENCH_MODEL_PATTERNS}"
        )
