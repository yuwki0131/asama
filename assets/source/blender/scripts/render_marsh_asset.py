"""Headless Blender entry point for the MARSH (terrain wetland) family.
...
Same camera rig, same CLI contract:
    ...
    blender --background --factory-startup --python render_marsh_asset.py -- \
        --model terrain-marsh-connected-0101 \
        --canvas 64x32 \
        --anchor 32,16 \
...
from render_asset_lib.marsh.registry import resolve_model, MARSH_MODEL_PATTERNS
...
def main() -> None:
    ...
    if builder is None:
        raise SystemExit(
            f"unknown marsh model: {args.model}; patterns: {MARSH_MODEL_PATTERNS}"
        )
