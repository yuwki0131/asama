...
export const steppedFortressScenario: ContentScenarioDefinition = {
  ...
  initialBuildings: [
    ...
    // L1縁の鉢巻塀: 段丘リムが裸のままだと、遠目に石垣ラインが「建物も囲いも
    // ない場所に取り残された薄い孤立壁のV字」に読める(Astra指摘watch)。
    // 塀を巡らせて「郭の縁」として意味づける。開口は東の大手門(70,60..62)のみ。
    // 西縁 x=24 は L1→L2→L3 と段を駆け上がる登り塀。南向き段差の崖セル
    // (24,67)/(24,69)は建物不可、間の1セル棚(24,68)も張らない(1セル塀断片が
    // 近接ズームで孤立柱に読めるため・L2指摘) — 塀が段の崖面に突き当たって
    // 再開する納まりで、リムの周回は視覚的に閉じる。
    ...hLine("wall", 24, 70, 52),
    ...vLine("wall", 70, 53, 70, [60, 61, 62]),
    ...hLine("wall", 24, 69, 70),
    ...vLine("wall", 24, 53, 69, [67, 68, 69]),
  ...
  initialUnits: [
    ...
    { type: "archer", position: { x: 36, y: 59 }, owner: "player" },
    ...
    { type: "spear_ashigaru", position: { x: 52, y: 60 }, owner: "player" },
    { type: "archer", position: { x: 49, y: 59 }, owner: "player" },
    ...
    { type: "spear_ashigaru", position: { x: 69, y: 61 }, owner: "player" },
    { type: "archer", position: { x: 68, y: 59 }, owner: "player" },
    { type: "sword_ashigaru", position: { x: 66, y: 62 }, owner: "player" },

    // 敵の物見 (東の街道)。
    { type: "spear_ashigaru", position: { x: 74, y: 88 }, owner: "enemy" },
    { type: "archer", position: { x: 76, y: 91 }, owner: "enemy" },
    { type: "spear_ashigaru", position: { x: 76, y: 89 }, owner: "enemy" },
    { type: "archer", position: { x: 72, y: 90 }, owner: "enemy" },
  ],
  waves: [
    {
      // 第1波: 東街道の前哨。
      ...
      spawns: [
        ...
        { type: "spear_ashigaru", position: { x: 73, y: 93 } },
        { type: "spear_ashigaru", position: { x: 77, y: 93 } },
    ...
    },
    {
      ...
      spawns: [
        ...
        { type: "spear_ashigaru", position: { x: 72, y: 93 } },
        { type: "sword_ashigaru", position: { x: 74, y: 93 } },
        { type: "archer", position: { x: 75, y: 94 } },
    ...
    },
    {
      ...
      spawns: [
        ...
        { type: "spear_ashigaru", position: { x: 77, y: 93 } },
        { type: "archer", position: { x: 75, y: 96 } },
    ...
    },
    {
      ...
      spawns: [
        ...
        { type: "spear_ashigaru", position: { x: 75, y: 94 } },
        { type: "sword_ashigaru", position: { x: 72, y: 92 } },
        { type: "archer", position: { x: 75, y: 95 } },
        { type: "spear_ashigaru", position: { x: 78, y: 92 } },
