...
export function isPassable(world: WorldState, coord: CellCoord, perspective?: "player" | "supply"): boolean {
  ...
  if (building !== null) {
    ...
    // 3-cell gates (wide_3 / narrow_3, both orientations): when passable, only
    // the center cell (footprint[1]) acts as the opening. The two end cells are
    // gate pillars/flanking walls and remain impassable regardless of
    // open/closed state.
    if (
