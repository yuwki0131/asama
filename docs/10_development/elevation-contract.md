...
- **1段 = 画面Y 24px 上方オフセット**(`ELEVATION_PIXELS_PER_LEVEL = 24`)。TILE_HEIGHT=32px に対し 0.75 タイル分で、崖面が読める高さかつ後列タイルを完全に隠さない。セル・建物・ユニット・装飾すべて `screenY -= elevation * 24`
- 坂セル上のユニットは中間高さ `-(elevation + 0.5) * 24 = -12px` 追加を基準に、ステップ補間で滑らかに遷移
...
| 坂の側面(坂セル側縁の崖) | `terrain.slope.<skin>.<n\|e\|s\|w>.side.<s\|e>` | `terrain.slope.cliff.n.side.e` |

`h<n>` は段差の大きさ(1段=24px、h2=48px、h3=72px の面)。バリエーション(`.v1` 等)は既存規約どおり末尾に付けてよい。
