...
| `slopeHalf` | `"lower" \| "upper" \| undefined` | `undefined` | 2セル緩坂の半段マーカー(坂セルのみ)。`undefined` = 従来の1セル急坂 |
...
### 2セル緩坂(gentle slope, `slopeHalf`)

進行方向に連続する2セルで1段を登るなだらかな坂。各セルが**半段 (0.5レベル)** を受け持つ:

- `slopeHalf: "lower"`(基準高さ `L`): 下端エッジ = `L`、上端エッジ = `L + 0.5`
- `slopeHalf: "upper"`(同じく `elevation = L`): 下端エッジ = `L + 0.5`、上端エッジ = `L + 1`(上段の面に接続)
- エッジ高一致規則はそのまま適用されるため lower ↔ upper ↔ 上段平地 が連結し、側面2方向は従来どおり崖(通行不可)
- 歩行面(描画)は lower = `L + 0.25`、upper = `L + 0.75` の中間高。登坂ペナルティは上りステップごと(平地→lower→upper→上段の3上りステップ)
- 戦闘判定は従来どおり `elevation`(低い側)を使う
- シナリオ語彙: `ScenarioSlope.length?: 1 | 2`(`position` は最下段セル)。地形建築: `placeSlope` コマンドの `length?: 1 | 2`(緩坂 50G、2セルとも空き平地・建物なし・同レベル・その先が +1 段であること)。`removeSlope` はどちらの半段を指しても**ペアごと**撤去
- タイル: cliff スキン(土の切り通し)のみ `terrain.slope2.dirt.<n|e|s|w>.<lower|upper>` の8枚(キャンバス 64x72、アンカー (32,56)、各セル20px立ち上がり)。ishigaki の石段は1セル急坂のまま(石段は急でも自然)
